...
| 6 | `apps/web` Playwright + `seed-web-e2e.ts` not in CI; prod `/api` edge-routing; `apps/web` deploy | Infra, non-checkbox — now being worked as **Batch S — deploy prep** (see the fix backlog). **Partial (Batch R5):** the `apps/web` *code* side of the `/api` contract is fixed — `src/lib/api.ts` `call()` now prepends `/api` (it was hitting bare `/feed` → 404 since Batch F added the global prefix), and the dev Vite proxy stops stripping `/api` (Nest owns the prefix now). **Batch S so far:** `nlp-service/Dockerfile` (deliverable 1). Still owed: main `Dockerfile` worker/cron cmds, entrypoint shim, `stack.prod.yaml`, `cloudflared` ingress (`/api/*` → Nest, rest → Astro — replaces nginx), `.env.production.example`, deploy CI job, `docs/deploy.md`, the CI Playwright job. |
...
### Batch S — deploy prep (deferred item #6) — IN PROGRESS (fix/batch-a-safety)
