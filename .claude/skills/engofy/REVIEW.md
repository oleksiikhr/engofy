...
- [x] **7 — `src/migrate.ts` migration entrypoint** — `MikroORM.init(mikro-orm.setup config)` → `orm.migrator.up()` → exit (non-zero on failure). Uses `@mikro-orm/postgresql` + `@mikro-orm/migrations` (both prod deps), so it runs in the --prod runtime image where `pnpm migration:up` (mikro-orm CLI + `@swc-node/register`, devDeps) cannot. Compiles to `dist/migrate.js`; `node migrate` is the pre-rollout step (invoked via `docker run --rm` in the CI deploy job + runbook — not a Swarm service, which handles one-shots poorly). Dockerfile header now lists five entrypoints. CI's `pnpm migration:up && pnpm migration:check` stays the drift gate (D17). Gate: type + biome + `pnpm test` (131/774, unchanged) + `pnpm test:cov` (90.31/78.5/86.86/90.69 vs 80/80/70/80 — pass) + `pnpm build` + `git diff --exit-code src/metadata.ts` clean. E2E verified: `node dist/migrate.js` against a fresh Postgres applied 21 migrations then reported `no pending migrations` on re-run, exit 0 both times.
- [x] **8 — CI**
