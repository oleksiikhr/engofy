...
| 6 | `apps/web` Playwright + `seed-web-e2e.ts` not in CI; prod `/api` edge-routing; `apps/web` deploy | Infra, non-checkbox — now being worked as **Batch S — deploy prep** (see the fix backlog). **Partial (Batch R5):** the `apps/web` *code* side of the `/api` contract is fixed — `src/lib/api.ts` `call()` now prepends `/api` (it was hitting bare `/feed` → 404 since Batch F added the global prefix), and the dev Vite proxy stops stripping `/api` (Nest owns the prefix now). **Batch S:** all 11 deliverables landed in-repo (Dockerfiles + tini + secret shim, `infra/stack.prod.yaml`, `infra/cloudflared/config.yml`, `infra/deploy.sh` (migrate via the existing `node cli migrate up`), `infra/pg-backup.sh`, `.env.production.example`, CI `apps-web` gate + `deploy.yaml` + `web-e2e.yaml`, `docs/deploy.md`). **#6 closes when actually deployed** — remaining is execution: provision the VM, create the secrets/tunnel, cut the first `v*` tag, and the first real `deploy.yaml` + `web-e2e.yaml` runs (both untested on a live runner). |
...
### Batch S — deploy prep (deferred item #6) — CODE/CONFIG DONE (fix/batch-a-safety)
Production deploy to Hetzner + Docker Swarm (single node), all inbound via one Cloudflare Tunnel (`cloudflared` as a Swarm service), Hetzner firewall deny-all. Not a code-review batch — infra + a few small `src/` changes. Decisions taken with the user (2026-09-06):
- **Postgres** — self-hosted Swarm service, pinned, local volume; backup cron `pg_dump` → Cloudflare R2.
- **Object storage** — Cloudflare R2 (S3-compatible; verify `S3_*` against the R2 endpoint).
- **Swarm** — single node; stack file + `docker secret` + rolling deploy, local overlay, no HA.
- **Registry** — GHCR: `ghcr.io/oleksiikhr/engofy{,-web,-nlp}`.
- **Swagger** — disabled in prod (code change, gate stays green).
- **Secrets** — bare `docker secret` + `docker-entrypoint.sh` shim (`/run/secrets/* → env`); nothing sensitive in the repo.
- **Domain** — `engofy.com`, single host (keeps `__Host-session` valid). `/api/*` → Nest, rest → Astro, path-routed in the `cloudflared` ingress rules (no nginx).
- **Server** — greenfield; the runbook covers provisioning through the first deploy.

Deliverables (one commit each, `pnpm test` gate `131 files / 774` for any `src/` / `test/` change; `pnpm build` + `git diff --exit-code src/metadata.ts` + `migration:up` + `migration:check` if entity/enum/migration touched):
- [x] **1 — `nlp-service/Dockerfile` + `.dockerignore`** — 2-stage (`python:3.14-slim-trixie`, matches the CI `nlp-service` job); deps + the pinned `en_core_web_sm` wheel resolved into `/opt/venv` in the build stage, no runtime `spacy download`. Runtime: non-root `USER 1000:1000`, `EXPOSE 8000`, `HEALTHCHECK` (stdlib `urllib` → `/health`), `uvicorn app:app --host 0.0.0.0`. Internal-only — `stack.prod.yaml` will declare no `ports:`. Validated: `docker build` green on 3.14 (spaCy 3.8.16 + model install fine), container `/health` + `/parse` correct, runs as uid 1000, Docker healthcheck reports `healthy`; image ~675 MB.
- [x] **2 — main `Dockerfile`** — `apk add --no-cache tini` + `ENTRYPOINT ["/sbin/tini", "--"]` (PID 1 signal-forwarding + orphan reaping, baked in so it does not depend on the Swarm runtime; Swarm `command:` still overrides CMD, so `tini -- <cmd>` holds). Header comment documents the four entrypoints (`node main` / `worker` / `cron` / `cli`, all bare from `WORKDIR /app/dist`, all present in `dist/`) and why there is no image-level `HEALTHCHECK` (only `main` serves HTTP — per-service `healthcheck:` goes in `stack.prod.yaml`). tini 0.19.0 verified present at `/sbin/tini` on `node:26.7.0-alpine3.24`. NOTE: could not run a full local image build — this host has no `buildx`/BuildKit and the (pre-existing) `RUN --mount=type=cache` lines need it; CI builds on GitHub Actions where BuildKit is default. Change is a 2-line addition to an already-working Dockerfile.
- [x] **3 — `docker-entrypoint.sh`** — POSIX `sh` shim: for each `/run/secrets/*` regular file, map the name to `UPPER_SNAKE` (`.`/`-` → `_`), and `export` its contents (trailing newlines stripped) **unless the var is already set** (explicit `-e` / compose `environment:` override wins), then `exec "$@"`. Copied to `/usr/local/bin/`, `chmod 0755`; `ENTRYPOINT` is now `["/sbin/tini", "--", "/usr/local/bin/docker-entrypoint.sh"]` with `CMD ["node", "main"]`. Only the Node image gets it (`nlp-service` has no secrets). Tested in a throwaway busybox container: `engofy_db_password`/`resend-api-key` secrets exported correctly, a pre-set `PUBLIC_URL` not clobbered by a `public_url` secret, `exec` chains the command.
- [x] **4 — `infra/stack.prod.yaml`** — 8 services: `web` (×2, `/_healthz` healthcheck, `start-first`), `worker` (×1, `stop_grace_period: 120s` for the ~2 min AI stream), `cron` (**×1, `order: stop-first`** — two = double Telegram getUpdates), `nlp` (×1, internal, image HEALTHCHECK), `apps-web` (×2, `API_ORIGIN=http://web:8080`), `cloudflared` (×2, mounts the ingress config + credentials, only inbound path), `postgres` (×1, `pgdata` volume, `POSTGRES_PASSWORD_FILE`, `max_connections=150`, `TZ=UTC`), `redis` (×1, no persistence, `allkeys-lru` 192mb). Encrypted overlay `engofy`; nothing published to the host. Credentials via 8 external `docker secret`s mounted with `target:` → `docker-entrypoint.sh` → `UPPER_SNAKE` env; non-secret site config via `env_file: ./.env.production`; fixed topology inline. YAML anchors for shared image refs / nest env / nest secrets / stateless `deploy`. Validated: `docker compose config` **and** `docker stack config` both exit 0, no warnings; `web` renders with merged env + targeted secrets + healthcheck + `ghcr.io/oleksiikhr/engofy:latest`.
- [x] **5 — `infra/cloudflared/config.yml`** — file-based tunnel config (ingress rules in the repo, not the dashboard). `engofy.com` single host; rule 1 `path: ^/api(/.*)?$` → `http://web:8080` (Nest owns the `/api` prefix, path forwarded unchanged), rule 2 catch-all → `http://apps-web:4321`, rule 3 `http_status:404`. `protocol: auto`, `metrics: 0.0.0.0:2000`, `originRequest.connectTimeout: 30s` (AI-backed calls). Loaded as the `engofy_cloudflared_config` Docker config; `<TUNNEL_ID>` + the `engofy_cloudflared_credentials` secret come from `cloudflared tunnel create` (runbook). This is the in-repo half of deferred #6 edge-routing — closed once actually deployed.
- [x] **6 — `.env.production.example`** — full matrix split three ways: (1) non-secret site config → `infra/.env.production` (git-ignored copy, read by `docker stack deploy`) — `IMAGE_TAG`, `PUBLIC_URL` (REQUIRED, `main.ts` throws without it), `LOG_LEVEL`, `SENTRY_RELEASE`/`SENTRY_DEBUG`, R2 `S3_ENDPOINT`/`S3_BUCKET`/`S3_REGION=auto`, `MAIL_FROM_EMAIL`, `AI_MODEL`, `GOOGLE_CLIENT_ID`/`PUBLIC_GOOGLE_CLIENT_ID`, `TELEGRAM_ADMIN_USER_ID`/`TELEGRAM_CHANNEL_ID`, commented tuning knobs; (2) secrets → `docker secret create` (8, names + `→ ENV_VAR` mapping listed, plus the `engofy_cloudflared_config` Docker config); (3) fixed topology → already in `stack.prod.yaml`, listed for reference. Every var cites its source file. NOTE: `MAIL_USE_MAILHOG` deliberately absent (prod uses Resend).
- [x] **7 — migrations at deploy = `node cli migrate up`** — the pre-rollout step runs the app's **existing** `nest-commander` CLI (`src/entrypoints/cli/migrate/migrate-up.command.ts` → `orm.migrator.up()`; `migrate down` also present). `nest-commander` is a **prod** dependency and `dist/cli.js` is in the runtime image, so `node cli migrate up` works there; `pnpm migration:up` does not (mikro-orm CLI + `@swc-node/register` are devDeps). `infra/deploy.sh` runs it as a one-shot Swarm service (`--restart-condition none`, on the overlay, `--secret engofy_db_password:mikro_orm_password` → shim → `MIKRO_ORM_PASSWORD`), polls the task to `Complete`, dumps logs, `service rm`, aborts the deploy on failure. CI's `pnpm migration:up && pnpm migration:check` stays the drift gate (D17). E2E verified: `node dist/cli.js migrate up` against a fresh Postgres applied all 21 migrations (`MigrateUpCommand` → "Applied 21 migration(s)"), no-op + exit 0 on re-run. **NOTE (cleanup):** an earlier version of this deliverable added a redundant `src/migrate.ts` (`MikroORM.init` + `migrator.up()`) — removed once the pre-existing CLI command was found; the AskUserQuestion that led to it was posed on a false premise (I'd only checked `pnpm migration:up`, not `src/entrypoints/cli/`).
- [x] **8 — CI**
  - **`app.yaml` — new `apps-web` job**: `pnpm --dir apps/web run type` (astro check) + `lint:check` (biome) + `build`. Puts the in-repo Astro frontend into the push gate (was outside tsc/biome entirely).
  - **`deploy.yaml` (new)** — `on: push tags v* | workflow_dispatch`, `concurrency: deploy-production`. `prepare` job → `IMAGE_TAG` (the tag, or `sha-<12>` for dispatch). `images` job → matrix build+push `ghcr.io/oleksiikhr/engofy{,-web,-nlp}:{$TAG,latest}` (buildx, gha cache per image, `GITHUB_TOKEN`). `deploy` job (`environment: production`) → `scp` `stack.prod.yaml` + `deploy.sh` to the manager, then `ssh`: `docker login ghcr.io` (PAT) + `IMAGE_TAG=… infra/deploy.sh`. Needs repo secrets `DEPLOY_SSH_{HOST,USER,KEY}` + `GHCR_{USERNAME,TOKEN}` (documented in the workflow header + runbook). Uses `appleboy/ssh-action`+`scp-action` (noted as swappable).
  - **`infra/deploy.sh` (new)** — the rollout both CI and a human on the box run: `docker pull` the Nest image, run migrations as a **one-shot swarm service** (`--restart-condition none`, on the `engofy_engofy` overlay, `--secret engofy_db_password:mikro_orm_password` → the entrypoint shim → `MIKRO_ORM_PASSWORD`, `node cli migrate up`), poll its task state, dump its logs, `service rm`, fail the deploy if it did not `Complete`; then `docker stack deploy --with-registry-auth --prune`. `bash -n` clean.
  - **`web-e2e.yaml` (new)** — the deferred-#6 Playwright job. `on: workflow_dispatch | push main` (not every push — heavy, and `main` doesn't exist yet, so effectively dispatch-only until first launch). postgres+redis service containers; `pnpm build` + `apps/web build`; `pnpm migration:up`; start `node dist/main` + the Astro server with curl-poll waits; `playwright install --with-deps chromium`; `pnpm --dir apps/web run test:e2e` (the suite's own `global-setup.ts` runs `test/e2e/seed-web-e2e.ts`); upload `playwright-report` on failure. **Untested on a real runner** — may need a tweak first run (weaker frontend verification, per the batch note).
  All 4 files: YAML parses; `app.yaml` existing jobs untouched. Pre-existing `.github/workflows/docker.yaml` (hadolint + buildx build + Trivy scan of the root `Dockerfile`, on main/PR/weekly) and `security-audit.yaml` (`pnpm audit`) are unaffected — `deploy.yaml` is a separate release lane (tag-triggered, pushes to GHCR). hadolint run locally on all three Dockerfiles (root incl. the `tini=~0.19.0` pin, `apps/web`, `nlp-service`): 0 findings.
- [ ] **9 — `docs/deploy.md`** — runbook: provision, first deploy, rolling deploy, rollback, Postgres backup/restore (→ R2), secret rotation.
- [x] **10 — small code changes**
  - **`/_healthz` split** — was one readiness probe (`health.check([db, redis])`); a Redis outage (Redis backs only the ephemeral throttler window + `otp:ip` counters) 503'd it and would make Swarm crash-loop every `web` task. Now `GET /_healthz` = liveness (`{ status: 'ok' }`, no deps — the container HEALTHCHECK target) and `GET /_healthz/ready` = readiness (db + redis, 503 via `HealthCheckErrorFilter`). `configure-app.ts` excludes both from the `/api` prefix. `stack.prod.yaml` `web` healthcheck now hits liveness (`retries: 3`, `interval: 15s`). ispecs: `health.controller.ispec` split into liveness + readiness cases (+1); `error.filter.ispec` moved its forced-500 to `/_healthz/ready` (still exercises the mocked `HealthCheckService`). `src/metadata.ts` regenerated (`HealthController` `check` → `live` + `ready`, deterministic, committed).
  - **`TRUST_PROXY` semantics** — `parseTrustProxy('1')` returned `['1']` (a bogus IP list → Fastify trusts nothing), not the hop count the deploy assumed. Added: a bare non-negative integer → `Number(token)` so `TRUST_PROXY=1` is "trust 1 proxy back from the socket" (proxy-addr semantics) — correct behind the single `cloudflared` hop, and resilient to the tunnel egress IP not being stable. `trust-proxy.helper.spec` +2 cases. `stack.prod.yaml` already sets `TRUST_PROXY: "1"`.
  - **Swagger in prod** — no code change needed: `main.ts:48` already guards the whole `SwaggerModule.setup` with `if (!isProdEnvironment())`, so `/_swagger` is not served when `NODE_ENV=production`. `cloudflared` has no route to it either. Documented in `docs/deploy.md`.
  - Gate: type + biome + `pnpm test` (**131 files / 777 tests**, was 131/774: +1 health split, +2 trust-proxy) + `pnpm test:cov` (exit 0) + `pnpm build` + `git diff src/metadata.ts` = the one expected `HealthController` line, committed.
- [x] **11 — `infra/pg-backup.sh` + R2 wiring check** — host cron script (swarm manager node): finds the running `engofy_postgres` task by swarm label, `pg_dump -Fc --no-owner` to a local `.dump`, sanity-checks size, streams it to `s3://$R2_BUCKET/pg/…` via a throwaway `amazon/aws-cli` container (`--endpoint-url $R2_ENDPOINT`, `AWS_DEFAULT_REGION=auto`), prunes local dumps > `KEEP_LOCAL_DAYS`. Remote retention = an R2 lifecycle rule (documented, not scripted). Restore one-liner (`pg_restore --clean --if-exists`) in the header + runbook. `bash -n` clean. **R2 wiring verified by inspection** (no live R2 creds here): `S3Service` uses only `PutObjectCommand` + `GetObjectCommand` (+ `NoSuchKey`) — all R2-supported; `s3.module.ts` sets `endpoint` + `forcePathStyle: !!endpoint` (→ true) + `region` — the documented-correct `@aws-sdk/client-s3` v3 R2 config with `S3_ENDPOINT=https://<acct>.r2.cloudflarestorage.com` + `S3_REGION=auto`. Footnote in `docs/deploy.md`: if a PUT 501s on checksum, set `requestChecksumCalculation: 'when_required'`.
