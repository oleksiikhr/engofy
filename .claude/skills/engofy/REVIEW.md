...
- [ ] D8: `PostQueueBootstrapService` = single `createQueue` authority (all `QueueName`s, shared opts const); `WorkerRegistrarService` only `boss.work()`
- [ ] D4: run row `Pending`+`startedAt` on entry (in `JobWorkerHost`); `Failed`+`errorMessage`+`retryCount++` on caught error before rethrow; `PostStatus.Failed` on retry exhaustion; per-queue `retryLimit`+backoff; `deadLetter` queue for paid AI stages; `Running` derived (no enum value)
- [ ] D5: `RetryPostHandler` also `nativeDelete` `Sentence`/`SentenceToken`/`GrammarMatch`/`Exercise` + null `PostPart.annotatedAt` (one flush)
- [ ] D6: `PublishPostHandler` no-ops-and-requeues until `PostPipelineRun(stage=Annotation, status=Completed)`
- [ ] `JobWorkerHost.work` → `Promise.allSettled` + per-job settle

### Batch D — schema reconciliation (D12) — migrations
- [ ] **§9 attribution** — `PostSource` + `attributionText` (NOT NULL) + `PostSourceType` enum + CHECK; feed/detail views + ingest paths + telegram `/add`
- [ ] `grammar_matches` composite `@Unique(sentenceId, grammarUsagePointId, tokenStart, tokenEnd)` (keep handler delete-by-sentence too)
- [ ] `posts.status`: `Annotating`/`Annotated` → single `Processing`
- [ ] move `Subscription` + enums → `modules/billing/entities/`; drop `SubscriptionStatus.Expired`
- [ ] rename `telegram_updates.telegram_message_id` → `update_id` (col + field) — coordinate with Batch G
- [ ] drop redundant standalone `@Index()` (`sentence`, `sentence_token`, `learning_card.userId`); add `@Index(['userId','due'])` on `learning_cards`
- [ ] `review_logs` — drop `created_at`
- [ ] wire `parseDoc(assembleDocFromParts(...))` at `get-post-detail` reassembly; fix `node-tree.type.ts` / `post-part-body.type.ts` comments
- [ ] add `pnpm migration:check` + CI step (D17)

### Batch E — learning / billing (D2, D11)
- [ ] D11: `get-profile` computes `masteryScore` at read time; column display-only; `recordGrammarReview` stops recomputing
- [ ] D2: `AddCard` / `ReviewCard` / `ActivateMockSubscription` return `id` / plain view (not the entity)
- [ ] `add-card` + `skill-progress.loadOrCreate` → `em.upsert(… onConflictAction:'ignore')` (race → 500 fix)
- [ ] `get-profile.computeStreak` → SQL `select distinct (reviewed_at at time zone 'UTC')::date`
- [ ] billing: `SubscriptionService` out of `exports`; add `GetSubscriptionQuery`
- [ ] `disableIdentityMap: true` on `learning` + `post` query handlers (DP2)

### Batch F — web infra (D14, D17)
- [ ] move `SessionAuthGuard` `APP_GUARD` + `ETagInterceptor` `APP_INTERCEPTOR` → `web.module.ts`
- [ ] `setGlobalPrefix('api', { exclude: ['_healthz'] })` + `.addServer('/api')` in the OpenAPI builder
- [ ] `@nestjs/throttler` `ThrottlerGuard` (global, Redis storage)
- [ ] web response DTOs independent of `modules/post` view types + mapper in `ContentController`; shared `{ items, nextOffset }` list envelope
- [ ] `/_healthz` — Terminus DB + Redis indicators + `@ApiTags('internal')`
- [ ] `HttpErrorFilter` → normalise `<500` body to `{ message }`
- [ ] query fields `.nullish().transform(v => v ?? undefined)` (empty-string-to-null vs `z.coerce` bug)
- [ ] `POST /learning/cards` → `@HttpCode(HttpStatus.OK)`
- [ ] `clearSessionCookie` mirrors `secure`/`httpOnly`/`sameSite`
- [ ] extract `configureApp(app)` shared by `main.ts` + `test/http/web/setup/create-app.helper.ts` (D17)
...
- [ ] `WORKER_QUEUES` → `Symbol()`
- [ ] `post ingest` — validate `--type` against `Object.values(PostType)` → `InvalidCliFlagError`; drop the no-op `parseTitle`
- [ ] bootstrap crash logs → `{ err }` not `{ cause: err }` (`worker.ts`, `cli.ts`, `cron.ts`)
- [ ] `post ingest <file>` — pre-check file exists

### Batch I — tests (D17)
- [ ] `test/fakes/{ai,nlp,mailer,telegram}.fake.ts` — one canonical `implements` per port; migrate the 8 bespoke fakes
- [ ] dedicated test Redis DB index + `FLUSHDB` in `afterEach` of `useOrmSuiteLifecycle`
- [ ] direct specs: worker processors, `AnthropicClientService`, `ChallengeMailerService`/`MAILER`, `CardLimitService`, `SkillProgressService`, `nlp-service/app.py` (pytest)
- [ ] verify the 80/80/70/80 coverage gate is actually green
- [ ] delete dead: `test/helpers/factory.helper.ts`, `@faker-js/faker` dep, `makeChangeSet`/`makeFlushArgs`
