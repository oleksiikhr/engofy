...
- [ ] **4 — `stack.prod.yaml`** — web/worker/cron/nlp/apps-web/cloudflared/postgres/redis; `deploy` (replicas, `update_config` order — `start-first` for stateless, `stop-first`/`replicas: 1` for cron, `restart_policy`, `resources.limits`, `stop_grace_period` 90–120 s on worker), encrypted local overlay, `secrets`, `env_file`.
