...
- [ ] **7 — migrations in deploy** — one-shot `migrate` service or a documented `docker run … pnpm migration:up` step before rollout (CI already runs `migration:up && migration:check`).
