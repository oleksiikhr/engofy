...
- [ ] **2 — main `Dockerfile`** — confirm/add `worker` + `cron` run commands (`node dist/worker` / `node dist/cron`), `--init`/tini for signal delivery, optional `HEALTHCHECK`; multi-stage already in place.
