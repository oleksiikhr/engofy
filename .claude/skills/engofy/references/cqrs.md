...
| Q3 | Deferred writes use `em.create` / `em.persist` / `em.remove`, or mutating a `findOne`-loaded entity (committed by the facade flush). `ChallengeService.issue` (Batch O) is `findOne`-then-mutate-or-`em.create`, not `em.upsert` — the challenge row must ride the same flush as its email job (Q4). | `services/session.service.ts:20`; `services/challenge.service.ts` `issue` |
| Q4 | Jobs are enqueued with `OutboxSenderService.send(em, QueueName, data)` — staged in a `WeakMap`, drained on the flush's `afterFlush` so the enqueue rides the write transaction. Anything the job needs (e.g. the OTP challenge row) must therefore be a **deferred** write, not an immediate one. | `commands/request-login-code/request-login-code.handler.ts:31-35`; `core/queue/outbox-sender.service.ts:20-53` |
| Q5 | Immediate writes (`em.nativeDelete`, raw `em.getConnection().execute`, `em.upsert`) auto-commit and do **not** ride the facade flush, so they break Q4 atomicity. Use them only when running *before* the flush is the actual intent — e.g. the OTP-lockout `nativeDelete` must land even though `consumeByOtp` then throws. Never for a row a queued job depends on. | `services/challenge.service.ts` (`consumeByOtp` lockout delete, `incrementAttempts`) |
| Q6 | Command results are plain values/DTOs (`LoginResult`, `ResolvedSession`, `CardView`, `SubscriptionView`, `IngestedPostView`, `void`), never a managed entity. A shared result type lives in `<module>/types/*.type.ts` with a `to<View>(entity)` mapper. `AddCard` / `ReviewCard` / `ActivateMockSubscription` (Batch E) and `IngestPost` (Batch N) all return views. | `auth/types/login-result.type.ts`; `learning/types/card-view.type.ts`; `billing/types/subscription-view.type.ts`; `post/types/ingested-post-view.type.ts` |
