...
.PHONY: ports
ports: ## Isolate this worktree's backend/web port/DB/Redis DB for offset N, e.g. `make ports OFFSET=1`
	...
	@touch .env.development.local .env.test.local apps/web/.env.local
	...
	set_var PORT $$WEB_PORT apps/web/.env.local; \
	set_var API_ORIGIN http://localhost:$$PORT apps/web/.env.local; \
	echo "----------------------------------------"; \
	echo " Worktree offset:  $$N"; \
	echo " Backend port:     $$PORT"; \
	echo " apps/web port:    $$WEB_PORT"; \
	echo " Postgres (dev):   $$DB_DEV"; \
	echo " Postgres (test):  $$DB_TEST"; \
	echo " Redis DB (dev):   $$REDIS_DB_DEV"; \
	echo " Redis DB (test):  $$REDIS_DB_TEST"; \
	echo " nlp-service:      not dockerized in dev — run it manually with:"; \
	echo "                    uvicorn app:app --port $$NLP_PORT"; \
	echo "----------------------------------------"
