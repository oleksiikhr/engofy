...
_all done). The former `REVIEW.md` D1–D18 decisions log has been folded into_
_each file's own rows (tagged `D1`, `D9`, etc. inline) and removed — see_
_`git log -- .agents/skills/engofy/REVIEW.md` for the original archive. Where a_
_rule file's own table has a non-struck row, the code does **not** yet match_
_the stated target — follow the target for new code._
