...
- `grammar_enrichment` is a branch off `ai_grammar` completion (parallel to
  ...
  have no `translations` entry for an `ENRICHMENT_LANGUAGES` language — one
  `completeStructured` call per point and language,
  ...
  also writes `translations[<lang>].explanation` (the explanation translated).
...
`pending` → `processing` → `published` / `failed`. There is a single `processing`
value, not per-annotation states — the pipeline has more stages than annotation, and
...
(default `original` / `'Original content'`).
