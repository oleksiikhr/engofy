...
| T3b | Fixtures are created **only** through factories — one `Factory` (`@mikro-orm/seeder`) per entity in `test/factories/<entity>.factory.ts`, reached as `suite.factories.<entity>` (or `factories(em).<entity>` inside a helper that takes `em`). `.createOne()` flushes, `.makeOne()` only persists. No `em.create(Entity, …)` / `new Entity()` + `persist` for an entity that has a factory. `definition()` is synchronous, deterministic (no faker; unique columns use `nextSeq()`) and never creates parents: the test creates the parent (`const post = await suite.factories.post.createOne()`) and passes its id (`postId: post.id`). Adding an entity means adding its factory and a key in `test/factories/factories.ts`. Helpers that compose several factories into a scenario ("post with a sentence and a token") are fine — they call the factories; a card / disposition / plan target needs a real word definition or grammar usage point, so use `makeWordDefinition(f)` / `makeGrammarUsagePoint(f, overrides)` from `test/helpers/reference-data.helper.ts` (they create the parents, unflushed). | `test/factories/`; `test/factories/factories.ispec.ts`; `test/helpers/reference-data.helper.ts` |
