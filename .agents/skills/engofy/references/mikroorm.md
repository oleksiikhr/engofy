...
| E5 | Enums stored as `text` + a CHECK constraint (not native pg enum); values are `snake_case` strings. | `Migration20260919151755.ts` |
...
| E7 | Every FK is a real DB foreign key, mapped without navigation: `@ManyToOne(() => X, { mapToPk: true, fieldName: '<col>_id', deleteRule: '<rule>' }) xId!: string` (`fieldName` is required — without it the column becomes `x_id_id`). `deleteRule`: `'cascade'` for rows owned by a user or post; `'restrict'` for reference data (`grammar_usage_points`, `grammar_constructions`, `words`, `word_definitions`, `phrases`). Exceptions, bare `@Property({ type: 'uuid' })`: `auth_sessions.user_id`, and `auth_challenges` (keyed by email, no `*_id`). Nullable FK → add `nullable: true`. Integration fixtures must create real parent rows via the factories (`factories(em).post.makeOne()`); random uuids violate the FK. | `modules/home/entities/daily-plan.entity.ts` `postId`; `entities/auth-session.entity.ts` `userId` (exception) |
...
`SubscriptionService` (Batch E), and the `post` query handlers `get-post-detail` / `get-grammar-construction` / `get-grammar-reference`
