...
// Word/phrase popup content for the reader — shared by the client popup
// (bundled into the page) and the /partials/lexicon-action HTMX response, so
// the action row renders identically before and after a save. Output is
// injected as HTML; interpolated values are escaped.
...
// exactly the ones `renderDoc` marks with a span.
...
export function lexiconActionsHtml(
  ...
  const field = target.kind === 'word' ? 'wordDefinitionId' : 'phraseId';
...
export function lexiconCardHtml(entry: LexiconEntry): string {
  ...
  const cefr = entry.cefrLevel
    ...
    : '';
  return `<div class="lex-popup__head">
