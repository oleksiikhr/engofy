...
export class DeleteExpiredAccountsService {
  ...
  private async deleteAccount(
    ...
    return this.em.transactional(async (em) => {
      ...
      // Every other user-owned table cascades from `users`; sessions carry no
      // FK, so they are removed here. A new user-owned table needs a cascading
      // FK to `users` (the ispec fails otherwise).
