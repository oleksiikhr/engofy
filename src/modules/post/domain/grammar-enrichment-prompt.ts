...
export function buildGrammarEnrichmentSystemPrompt(
  language: ContentLanguage,
): string {
  const { name } = CONTENT_LANGUAGE_INFO[language];
  return `You write short grammar help for learners of English in a CEFR-graded reading app.
...
- "translatedExplanation": the same explanation written in natural ${name} (a faithful translation, not a paraphrase). Keep the construction pattern itself (e.g. "have/has + past participle") in English.
...
export const grammarEnrichmentToolSchema = z.object({
  ...
  translatedExplanation: text.max(800),
...
export function toGrammarEnrichmentContent(
  ...
  return {
    ...
    translatedExplanation: result.translatedExplanation.trim(),
    examples,
  };
}
