...
import { assembleDocFromParts, splitDocIntoParts } from './content-parts.js';
...
describe('splitDocIntoParts / assembleDocFromParts', () => {
  it('splits a Doc into one part per paragraph and reassembles it unchanged', () => {
    ...
    expect(parts).toEqual([
      { blockIndex: 0, kind: ContentPartKind.Paragraph, body: doc.children[0] },
      { blockIndex: 1, kind: ContentPartKind.Paragraph, body: doc.children[1] },
    ]);
  ...
  it('keeps a whole ListBlock (all its items) as a single part', () => {
    ...
    expect(parts).toEqual([
      { blockIndex: 0, kind: ContentPartKind.List, body: doc.children[0] },
    ]);
    expect(assembleDocFromParts(parts)).toEqual(doc);
  });
  ...
  it('assembles a Doc from out-of-order parts by blockIndex', () => {
    const parts = [
      {
        blockIndex: 1,
        kind: ContentPartKind.Paragraph,
        body: {
